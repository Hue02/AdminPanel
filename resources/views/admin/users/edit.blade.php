    ...
    <div class="row justify-content-center">
        <div class="col-md-6">
            <div class="card shadow-lg">
                ...
                <div class="card-body">
                    <form id="update-form" action="{{ route('admin.users.update', $user->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select name="grade_level" id="grade_level" class="form-control" required>
                                ...
                                <option value="7" {{ old('grade_level', $user->grade_level) == '7' ? 'selected' : '' }}>Grade 7</option>
                                <option value="8" {{ old('grade_level', $user->grade_level) == '8' ? 'selected' : '' }}>Grade 8</option>
                                <option value="9" {{ old('grade_level', $user->grade_level) == '9' ? 'selected' : '' }}>Grade 9</option>
                                <option value="10" {{ old('grade_level', $user->grade_level) == '10' ? 'selected' : '' }}>Grade 10</option>
                            </select>
                            @error('grade_level')
                                <div class="text-danger mt-1">{{ $message }}</div>
                            @enderror
...
<script>
    document.getElementById('update-btn').addEventListener('click', function (event) {
        ...
        // If grade changed, warn about progress reset
        if (selectedGrade !== origGrade) {
            Swal.fire({
                title: "Change Grade Level?",
                html: "<p>You've changed the student's grade level. <strong>This will reset the student's progress (level, coins, answers) to defaults.</strong></p>",
                icon: "warning",
