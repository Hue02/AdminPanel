...
<div class="container mt-4">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex flex-column flex-md-row gap-2">
            <!-- CSV Upload -->
            <form action="{{ route('teacher.trivia.upload') }}" method="POST" enctype="multipart/form-data" class="d-flex flex-column flex-sm-row gap-2">
                @csrf
            ...
            <!-- Add New Trivia -->
            <a href="{{ route('teacher.trivia.create') }}" class="btn btn-success btn-sm">
                    ...
                    <div class="card shadow-sm h-100 d-flex flex-column">
                        <div class="card-body d-flex flex-column">
                            ...
                            <div class="mt-auto d-flex justify-content-between">
                                <a href="{{ route('teacher.trivia.edit', $trivia->id) }}" class="btn btn-warning btn-sm">
                                    <i class="fas fa-edit"></i> Edit
                                </a>
                                <button class="btn btn-danger btn-sm" onclick="confirmDelete({{ $trivia->id }})">
                                    <i class="fas fa-trash-alt"></i> Delete
                                </button>
                                <form id="delete-form-{{ $trivia->id }}" action="{{ route('teacher.trivia.destroy', $trivia->id) }}" method="POST" style="display:none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </div>
    ...
    <a href="{{ route('teacher.trivia.create') }}" class="btn btn-success rounded-circle shadow-lg d-md-none position-fixed"
...
function searchTrivia() {
    ...
    let url = "{{ route('teacher.trivia.index') }}?search=" + encodeURIComponent(searchQuery);
...
function resetSearch() {
    window.location.href = "{{ route('teacher.trivia.index') }}";
}
